...
const STATUT = {
  ...
  EN_COURS: { label: "En cours",  bg: "#1B3A2D", color: "#26C2A1" },
  TERMINE:  { label: "Terminé",   bg: "#EEF2F7", color: "#6B7280" },
  ANNULE:   { label: "Annulé",    bg: "#2D1117", color: "#E11D48" },
};
    ...
    <div style={st.page}>
      ...
      <main style={st.main}>
        ...
        <section style={st.shortcuts}>
          <Btn label="Vendre un billet"        onClick={() => navigate("/receptionniste/vente")}     color="#26C2A1" />
          <BtnBadge
            ...
            color="#E67E22"
          />
          <Btn label="Chercher une réservation" onClick={() => navigate("/receptionniste/recherche")}  color="#304FFE" />
          <Btn label="Liste des passagers"      onClick={() => navigate("/receptionniste/passagers")}  color="#6E3FAA" />
          <Btn label="Historique des ventes"    onClick={() => navigate("/receptionniste/historique")} color="#EEF2F7" />
        </section>

        {/* Stats */}
        <section style={st.statsRow}>
          <StatCard label="Billets vendus auj." value={loading ? "…" : stats.billets_auj}           color="#26C2A1" />
                ...
                <tbody>
                  {trajets.map(t => (
                    <tr key={t.id} style={{ ...st.tr, ...(t.est_aujourd_hui ? { backgroundColor: "#0D1F17" } : {}) }}>
                      <td style={st.td}>
                        <span style={{ fontSize: "12px", fontWeight: "600", color: t.est_aujourd_hui ? "#26C2A1" : "#6B7280" }}>
                          {t.est_aujourd_hui ? "Aujourd'hui" : t.date}
                        </span>
                      </td>
                      <td style={st.td}><span style={st.busBadge}>{t.bus}</span></td>
                      <td style={st.td}>{t.ligne}</td>
                      <td style={st.td}><strong style={{ color: "#1A1348" }}>{t.depart}</strong></td>
...
function TauxBar({ taux }) {
  const color = taux >= 90 ? "#E11D48" : taux >= 60 ? "#F0883E" : "#26C2A1";
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
      <div style={{ width: "60px", height: "6px", backgroundColor: "#EEF2F7", borderRadius: "3px", overflow: "hidden" }}>
...
const st = {
  ...
  statCard:  { flex: "1 1 180px", backgroundColor: "#FFFFFF", borderRadius: "12px", padding: "20px 22px", borderTop: "3px solid", boxShadow: "0 2px 10px rgba(0,0,0,0.3)" },
  statLabel: { fontSize: "12px", color: "#6B7280", textTransform: "uppercase", letterSpacing: "1px", margin: "0 0 8px" },
  statValue: { fontWeight: "800", margin: 0, lineHeight: 1 },
  alertesBox:{ backgroundColor: "#FFFFFF", borderRadius: "10px", padding: "16px 20px", border: "1px solid #F0883E33" },
  alerte:    { display: "flex", alignItems: "center", gap: "10px", padding: "8px 0", borderBottom: "1px solid #EEF2F7", fontSize: "13px", color: "#C9D1D9" },
  ...
  busBadge:  { backgroundColor: "#EEF2F7", color: "#1A1348", padding: "3px 8px", borderRadius: "6px", fontSize: "12px", fontWeight: "700", fontFamily: "monospace" },
  badge:     { padding: "3px 10px", borderRadius: "20px", fontSize: "12px", fontWeight: "600" },
  empty:     { fontSize: "13px", color: "#6B7280", margin: 0 },
  btnSell:   { padding: "4px 12px", fontSize: "12px", fontWeight: "600", color: "#26C2A1", backgroundColor: "transparent", border: "1.5px solid #26C2A1", borderRadius: "6px", cursor: "pointer", fontFamily: "inherit" },
};
