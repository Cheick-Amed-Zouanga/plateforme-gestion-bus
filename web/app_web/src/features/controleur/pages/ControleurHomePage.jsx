...
export default function ControleurHomePage() {
    ...
    <div style={st.page}><Header /><SubHeader title="Tableau de bord — Contrôleur" />
      <main style={st.main}>
        <div style={st.emptyCard}>
          ...
          <h3 style={{ color: "#1A1348", margin: "0 0 8px", fontSize: "16px" }}>Aucun trajet assigné aujourd'hui</h3>
          <p style={{ color: "#6B7280", margin: 0, fontSize: "13px" }}>Vous n'avez pas de trajet prévu pour aujourd'hui.</p>
        </div>
      </main>
    </div>
  );
  ...
  const statusColor = STATUT_COLOR[trajet.statut] ?? "#6B7280";
      ...
      <main style={st.main}>
        ...
        <div style={{ ...st.card, borderTop: `3px solid ${statusColor}` }}>
          <div style={st.trajetHeader}>
            <div>
              <div style={{ fontSize: "12px", color: "#6B7280", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: "4px" }}>Trajet du jour</div>
              <h2 style={{ margin: "0 0 4px", fontSize: "20px", color: "#1A1348", fontWeight: "800" }}>{trajet.ligne_display ?? trajet.ligne}</h2>
              <span style={{ fontSize: "13px", color: "#6B7280" }}>{trajet.bus_display ?? trajet.bus}</span>
            </div>
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ fontSize: "13px", color: "#6B7280", marginTop: "8px" }}>
                Départ prévu : <strong style={{ color: "#1A1348" }}>
        ...
        <div style={st.statsRow}>
          ...
          <StatCard label="Billets validés" value={compteurs?.billets_valides ?? 0} color="#26C2A1" />
          <StatCard label="En attente paiement" value={compteurs?.en_attente_paiement ?? 0} color="#F0883E" />
          <StatCard label="Incidents" value={compteurs?.incidents ?? 0} color="#E11D48" />
        </div>

        {/* Actions rapides */}
        <div style={st.shortcuts}>
          <ActionBtn label="Scanner un billet"     color="#26C2A1" onClick={() => navigate("/controleur/valider")}                        icon="✓" />
          <ActionBtn label="Liste d'embarquement"  color="#0E7490" onClick={() => navigate(`/controleur/embarquement/${trajet.id}`)}     icon="👥" />
          <ActionBtn label="Suivi du trajet"        color="#304FFE" onClick={() => navigate(`/controleur/suivi/${trajet.id}`)}           icon="📍" />
            ...
            {(arrets ?? []).map((a, i) => (
              <div key={a.id} style={st.timelineItem}>
                <div style={st.timelineDotCol}>
                  <div style={{
                    ...st.timelineDot,
                    backgroundColor: a.est_depart ? "#26C2A1" : a.est_arrivee ? "#E11D48" : "#58A6FF",
                  }} />
                  {i < arrets.length - 1 && <div style={st.timelineLine} />}
                </div>
                <div style={st.timelineContent}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <span style={{ fontWeight: "700", color: "#1A1348", fontSize: "14px" }}>{a.ville}</span>
                  ...
                  {(a.est_depart || a.est_arrivee) && (
                    <span style={{ fontSize: "11px", color: a.est_depart ? "#26C2A1" : "#E11D48", fontWeight: "600" }}>
...
function StatCard({ label, value, color }) {
  return (
    <div style={{ flex: "1 1 160px", backgroundColor: "#FFFFFF", borderRadius: "12px", padding: "18px 20px", borderTop: `3px solid ${color}`, boxShadow: "0 2px 10px rgba(0,0,0,0.3)" }}>
      <p style={{ fontSize: "11px", color: "#6B7280", textTransform: "uppercase", letterSpacing: "1px", margin: "0 0 6px" }}>{label}</p>
...
const st = {
  ...
  timelineLine:  { width: "2px", flex: 1, backgroundColor: "#EEF2F7", margin: "4px 0", minHeight: "16px" },
  timelineContent:{ flex: 1, paddingBottom: "14px" },
  muted:         { color: "#6B7280", fontSize: "13px" },
  emptyCard:     { backgroundColor: "#FFFFFF", borderRadius: "12px", padding: "48px 24px", textAlign: "center" },
};
