...
export default function ChefTarifsCreerPage() {
  ...
  async function handleLigneChange(e) {
    ...
    try {
      ...
      setErreur("Impossible de charger les arrêts : " + e.message);
    } finally {
  ...
  const arretsArrivee = form.arret_depart
    ? arrets.filter(a => a.ordre > arrets.find(x => String(x.id) === form.arret_depart)?.ordre)
    : [];

  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.ligne)         { setErreur("Veuillez choisir une ligne.");          return; }
    if (!form.arret_depart)  { setErreur("Veuillez choisir une ville de départ."); return; }
    if (!form.arret_arrivee) { setErreur("Veuillez choisir une ville d'arrivée."); return; }
    if (!form.prix || Number(form.prix) < 0) { setErreur("Le prix est obligatoire et doit être ≥ 0."); return; }
    ...
    try {
      ...
      const dep = arrets.find(a => String(a.id) === form.arret_depart);
      const arr = arrets.find(a => String(a.id) === form.arret_arrivee);
      const typeLabel = form.type_bus === "VIP" ? "VIP" : "Standard";
      setSucces(`Tarif défini : ${dep?.ville} → ${arr?.ville} — ${typeLabel} — ${Number(form.prix).toLocaleString("fr-FR")} ${form.devise}`);
      setForm(prev => ({ ...prev, arret_depart: "", arret_arrivee: "", prix: "" }));
    } catch (e) {
      setErreur(e.message);
    } finally {
      setLoading(false);
  ...
  return (
    <div style={s.page}>
      <Header />
      <SubHeader title="Définir un tarif" />
      <main style={s.main}>
        <div style={s.card}>
          <h2 style={st.title}>Nouveau tarif par segment</h2>
          <p style={st.desc}>
            Sélectionnez la ligne, puis choisissez la ville de départ et d'arrivée du segment à tarifer.
          </p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {arrets.length > 0 && (
              <>
                <div style={s.group}>
                  ...
                  <select style={s.input} name="arret_depart" value={form.arret_depart} onChange={handleChange}>
            ...
            <div style={s.btnRow}>
              <button type="button" style={s.btnBack} onClick={() => navigate("/chef/tarifs")}>
                Retour
              </button>
              <button
                type="submit"
...
const st = {
  title: { fontSize: "18px", fontWeight: "700", color: "#E6EDF3", marginBottom: "6px" },
  desc:  { fontSize: "13px", color: "#8B949E", marginBottom: "24px" },
